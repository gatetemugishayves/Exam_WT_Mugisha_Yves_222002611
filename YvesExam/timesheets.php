...
<section>
    ...
    <h1><u> timesheets Form </u></h1>
    <form method="post" onsubmit="return confirmInsert();">
            
        <label for="tid">timesheet_id:</label>
        <input type="number" id="tid" name="tid"><br><br>

        <label for="eid">employee_id:</label>
        <input type="number" id="eid" name="eid"><br><br>

        <label for="rh">regular_hours:</label>
        <input type="number" id="rh" name="rh" required><br><br>

        <label for="oh">overtime_hours:</label>
        <input type="number" id="oh" name="oh" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO timesheets(timesheet_id, employee_id, regular_hours,overtime_hours) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $tid, $eid, $rh,$oh );
...
// SQL query to fetch data from the timesheets table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of timesheets</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of timesheets</h2></center>
    <table border="5">
        <tr>
            <th>timesheet_id</th>
            <th>employee_id</th>
      ...
      include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all timesheets
        $sql = "SELECT * FROM timesheets";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $tid = $row['timesheet_id']; // Fetch the timesheet_id
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_timesheets.php?timesheet_id=$tid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_timesheets.php?timesheet_id=$tid'>Update</a></td> 
                </tr>";
            }
        } else {
